...
| `ONR_DST`   | [State Diagram](diagrams/ONR_DST.png)                                                                                          | Жизненный цикл напоминания клиенту (диаграмма)                                                                        |
| `ONR_TST`   | [State Table](src/ONR_TST.xlsx)                                                                                                | Жизненный цикл оплаты услуги (таблица)                                                                                        |
...
- `TST` — State Table, таблица состояний;
...
[`ONR_TST.xlsx`](src/ONR_TST.xlsx) содержит таблицу состояний объекта
**«Оплата»**.

Таблица состояний описывает жизненный цикл оплаты услуги и показывает,
как объект переходит из одного состояния в другое в результате действий менеджера.

**Цель:** отразить жизненный цикл оплаты услуги и показать изменение её состояний
в результате действий менеджера.

**Область рассмотрения:** **TO BE**.

Основной жизненный цикл:

`Начислена → Принята → Передана в бухгалтерию`

Для каждого перехода в таблице указаны:

- предыдущее состояние;
- событие, переводящее объект в следующее состояние;
- инициатор события;
- условие перехода, если оно предусмотрено;
- последующее состояние.

| Предыдущее состояние    | Событие (действие)                                         | Инициатор | Условие              | Последующее состояние  |
| ------------------------------------------ | ------------------------------------------------------------------------- | ------------------ | --------------------------- | ------------------------------------------ |
| Начало                               | Начислить оплату за оказанную услугу      | Менеджер   | Услуга оказана | Начислена                         |
| Начислена                         | Принять оплату                                               | Менеджер   | —                          | Принята                             |
| Принята                             | Передать данные об оплате в бухгалтерию | Менеджер   | —                          | Передана в бухгалтерию |
| Передана в бухгалтерию | —                                                                        | —                 | —                          | Конец                                 |
